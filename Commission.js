...
var Commission = (function () {
  ...
  function metricasDoAtendente_(email, competencia) {
    ...
    // Tratativas por produto (retidas + canceladas) — base do % individual
    canceladas.concat(retencoes).forEach(function (r) {
      var k = 'TRATATIVAS_' + Utils.slug(r.produto);
      porProduto[k] = (porProduto[k] || 0) + 1;
    });
    ...
    // % de retenção INDIVIDUAL POR PRODUTO (ex.: PERCENTUAL_RETENCAO_CONTA,
    // PERCENTUAL_RETENCAO_CARTAO). Conta e Cartão nunca se misturam.
    CONFIG.PRODUTOS_RETENCAO.forEach(function (p) {
      var s = Utils.slug(p);
      var trat = Number(porProduto['TRATATIVAS_' + s] || 0);
      var ret  = Number(porProduto['RETENCOES_' + s] || 0);
      metricas['PERCENTUAL_RETENCAO_' + s] = trat ? Utils.round2((ret / trat) * 100) : 0;
    });
   ...
   * --------------------------------------------------------- */
  /* ---------------------------------------------------------
   * GARANTIA "CONTA UMA VEZ SÓ":
   * um mesmo registro não pode ser pago em duplicidade.
   *  1) Linhas de regra IDÊNTICAS (duplicadas na tabela)
   *     → somente a primeira vale.
   *  2) Várias regras para a MESMA métrica na mesma faixa
   *     → vale apenas a de melhor prioridade (menor número);
   *       empate: a mais específica (produto/canal preenchidos).
   *  3) Regra genérica de TOTAL (ex.: RETENCOES_TOTAL) não soma
   *     quando uma regra específica da mesma família já pagou
   *     (ex.: RETENCOES_CONTA) — senão a mesma retenção
   *     contaria duas vezes.
   * --------------------------------------------------------- */
  function chaveDuplicada_(r) {
    return [r.metrica, r.faixaMin, r.faixaMax, r.valor, r.unidade,
            r.canal || '', r.produto || '', r.tipo || ''].join('|');
  }

  function familia_(metrica) {
    if (/^VENDAS_/.test(metrica)) return 'VENDAS';
    if (/^RETENCOES_TIPO_/.test(metrica)) return null;      // adicional próprio (ex.: argumentação) — não conflita
    if (/^RETENCOES_/.test(metrica)) return 'RETENCOES';
    return null;
  }

  function especificidade_(r) {
    return (r.produto && r.produto !== '*' ? 1 : 0) +
           (r.canal && r.canal !== '*' ? 1 : 0) +
           (r.tipo && r.tipo !== '*' ? 1 : 0);
  }
  ...
  function calcularDigital_(dados, canal) {
    ...
    var regras = Db.readAll(CONFIG.SHEETS.REGRAS_COMISSAO)
      ...
      // (1) remove linhas duplicadas idênticas
      .filter(function (r) {
        var k = chaveDuplicada_(r);
        if (vistas[k]) return false;
        vistas[k] = true;
        return true;
      });

    // Candidatas: regras cuja métrica caiu na faixa
    var candidatas = regras.filter(function (regra) {
      var mv = Number(dados.metricas[regra.metrica] || 0);
    ...
    // (2) UMA vencedora por métrica: prioridade (menor nº) → especificidade → ordem
    var porMetrica = {};
    candidatas.forEach(function (r) {
      var atual = porMetrica[r.metrica];
      if (!atual) { porMetrica[r.metrica] = r; return; }
      var pa = Number(atual.prioridade || 0), pr = Number(r.prioridade || 0);
      if (pr < pa || (pr === pa && especificidade_(r) > especificidade_(atual))) {
        porMetrica[r.metrica] = r;
      }
    });

    // (3) TOTAL genérico sai quando há regra específica da mesma família
    var metricasVencedoras = Object.keys(porMetrica);
    var temEspecifica = {};
    metricasVencedoras.forEach(function (m) {
      var f = familia_(m);
      if (f && m !== f + '_TOTAL') temEspecifica[f] = true;
    });

    var itens = [], total = 0;
    metricasVencedoras.forEach(function (m) {
      var f = familia_(m);
      if (f && m === f + '_TOTAL' && temEspecifica[f]) return;   // já pago pela específica

      var regra = porMetrica[m];
      var metricaValor = Number(dados.metricas[m] || 0);
      var valor = 0;
      ...
      total += valor;
      itens.push({ regra: regra.nome, metrica: m,
  ...
  function calcularFone_(dados) {
    ...
    var regras = Db.readAll(CONFIG.SHEETS.REGRAS_FONE)
      ...
      // remove linhas duplicadas idênticas — cada regra vale uma vez
      .filter(function (r) {
        var k = [r.produto || '', r.tipo || '', r.percentualMin, r.percentualMax,
                 r.valor, r.unidade, r.canal || ''].join('|');
        if (vistas[k]) return false;
        vistas[k] = true;
        return true;
      });

    var percGeral = dados.metricas.PERCENTUAL_RETENCAO;
    var itens = [], total = 0;

    // Agrupa por produto+tipo: entre as faixas atingidas do mesmo grupo,
    // vale SOMENTE a mais alta (faixas sobrepostas não somam — a mesma
    // retenção nunca é paga duas vezes)
    var vencedoras = {};
    regras.forEach(function (regra) {
      // % de retenção é INDIVIDUAL POR PRODUTO: regra de Conta usa
      // PERCENTUAL_RETENCAO_CONTA; de Cartão, PERCENTUAL_RETENCAO_CARTAO.
      // Sem produto (ou '*'), usa o % geral.
      var perc = (regra.produto && regra.produto !== '*')
        ? Number(dados.metricas['PERCENTUAL_RETENCAO_' + Utils.slug(regra.produto)] || 0)
        : percGeral;
      ...
      var g = (regra.produto || '*') + '|' + (regra.tipo || '*');
      if (!vencedoras[g] || min > Number(vencedoras[g].percentualMin || 0)) {
        vencedoras[g] = regra;
        vencedoras[g]._min = min;
        vencedoras[g]._max = max;
      }
    });

    Object.keys(vencedoras).forEach(function (g) {
      var regra = vencedoras[g];
      var min = regra._min, max = regra._max;
      if (regra.unidade === 'POR_ITEM') {
        ...
        var chave = regra.produto && regra.produto !== '*'
          ...
          : 'RETENCOES_TOTAL';
        var base = Number(dados.metricas[chave] || 0);
    ...
    return {
      motor: 'FONE',
      percentualRetencao: percGeral,
      percentualRetencaoConta: dados.metricas.PERCENTUAL_RETENCAO_CONTA,
      percentualRetencaoCartao: dados.metricas.PERCENTUAL_RETENCAO_CARTAO,
  ...
  function calcularBonificacoes_(dados, canal) {
    ...
    var bonus = Db.readAll(CONFIG.SHEETS.BONIFICACOES).filter(function (b) {
      ...
      // linhas idênticas (duplicadas) valem uma única vez
      var k = [b.condicaoMetrica, b.condicaoMin, b.condicaoMax, b.valor,
               b.unidade, b.canal || '', b.produto || ''].join('|');
      if (vistas[k]) return false;
      vistas[k] = true;
      return true;
    });
